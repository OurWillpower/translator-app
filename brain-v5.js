...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- 2. Load Voices (THE 100% CORRECT FIX) ---
    function loadAndDisplayVoices() {
        voices = synthesis.getVoices();
        
        if (voices.length > 0) {
            
            // --- THIS IS THE "DICTIONARY" FIX ---
            ...
            const languageDictionary = {
                ...
                "vi": "Vietnamese", "el": "Greek", "fi": "Finnish"
            };
            ...
            for (const lang of languages) {
                // *** THIS IS THE FIX ***
                // We now use the dictionary. The broken "try...catch" is gone.
    ...
    muteButton.addEventListener("click", () => {
        ...
        if (isMuted) {
            ...
            iconSpeaker.style.display = "none"; // Hide speaker
            iconMute.style.display = "block"; // Show mute
            synthesis.cancel();
        } else {
            muteButton.classList.remove("muted");
            iconSpeaker.style.display = "block"; // Show speaker
            iconMute.style.display = "none"; // Hide mute
        }
    });
